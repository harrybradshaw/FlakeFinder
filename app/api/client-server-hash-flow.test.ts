    ...
    from: vi.fn((table: string) => {
      ...
      const createChainableMock = (finalData: any) => {
        const chain: any = {
          ...
          then: vi.fn((callback: any) => callback({ data: finalData, error: null })),
        };
        return chain;
      };
      ...
      if (table === 'test_runs') {
        ...
        return createChainableMock({ user_id: 'test-user-123', project_id: 'project-123' });
        ...
        return createChainableMock([{ organization_id: 'org-123', project_id: 'project-123' }]);
...
describe('Client-Server Hash Flow Integration', () => {
  ...
  it('DUPLICATE CHECK: should accept hash-only request (no file upload)', async () => {
    ...
    console.log('Request size: ~', calculatedHash.length, 'bytes (vs ~12MB for file)');
  ...
  it('UPLOAD: should use pre-calculated hash instead of recalculating', async () => {
    ...
    console.log('\n=== File Optimization ===');
    console.log('Original size:', (testReportFile.size / 1024 / 1024).toFixed(2), 'MB');
    console.log('Optimized size:', (optimizedFile.size / 1024 / 1024).toFixed(2), 'MB');
    console.log('Savings:', ((1 - optimizedFile.size / testReportFile.size) * 100).toFixed(1), '%');
    console.log('=========================\n');
  ...
  it('END-TO-END: complete flow with bandwidth savings', async () => {
    ...
    console.log('  From file:', testReportFile.name, '(', (testReportFile.size / 1024 / 1024).toFixed(2), 'MB)');
    ...
    const duplicateCheckRequest = new NextRequest('http://localhost:3000/api/check-duplicate', {
      method: 'POST',
      body: duplicateCheckData,
    });

    const duplicateCheckResponse = await checkDuplicatePOST(duplicateCheckRequest);
    ...
    console.log('  Bandwidth saved:', (testReportFile.size / 1024 / 1024).toFixed(2), 'MB');
    ...
    console.log('  Original:', (testReportFile.size / 1024 / 1024).toFixed(2), 'MB');
    console.log('  Optimized:', (optimizedFile.size / 1024 / 1024).toFixed(2), 'MB');
    ...
    console.log('  Upload size:', (optimizedFile.size / 1024 / 1024).toFixed(2), 'MB');
    console.log('  Success:', uploadResult.success);

    console.log('\n=== TOTAL BANDWIDTH SAVINGS ===');
    console.log('WITHOUT optimization:');
    console.log('  - Duplicate check:', (testReportFile.size / 1024 / 1024).toFixed(2), 'MB');
    console.log('  - Upload:', (testReportFile.size / 1024 / 1024).toFixed(2), 'MB');
    console.log('  - TOTAL:', ((testReportFile.size * 2) / 1024 / 1024).toFixed(2), 'MB');
    console.log('WITH optimization:');
    console.log('  - Duplicate check: ~0.001 MB (hash only)');
    console.log('  - Upload:', (optimizedFile.size / 1024 / 1024).toFixed(2), 'MB');
    console.log('  - TOTAL:', (optimizedFile.size / 1024 / 1024).toFixed(2), 'MB');
    const savings = ((testReportFile.size * 2 - optimizedFile.size) / (testReportFile.size * 2) * 100);
