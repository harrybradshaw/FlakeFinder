              ...
              const eqChain = {
                order: (field: string, options: any) => ({
                  limit: (n: number) => {
                    ...
                    const runs = Array.from(mockSupabaseData.values()).filter(
                      (item: any) => item.content_hash === value,
                    );
...
describe.skip("Integration: Duplicate Detection", () => {
  ...
  it("should NOT detect duplicate when different metadata is used", async () => {
    ...
    const file1 = new File([buffer1], "report1.zip", {
      type: "application/zip",
    });
    ...
    const file2 = new File([buffer2], "report2.zip", {
      type: "application/zip",
    });
    ...
    console.log(
      "✅ Correctly identified as non-duplicate due to different metadata",
    );
