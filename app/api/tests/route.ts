...
export async function GET(request: NextRequest) {
  try {
    ...
    const testsResponse = Array.from(testMetrics.values()).map((metrics) => ({
      ...
      passRate: metrics.totalRuns > 0 ? ((metrics.passed / metrics.totalRuns) * 100).toFixed(1) : "0.0",
      failRate: metrics.totalRuns > 0 ? ((metrics.failed / metrics.totalRuns) * 100).toFixed(1) : "0.0",
      flakyRate: metrics.totalRuns > 0 ? ((metrics.flaky / metrics.totalRuns) * 100).toFixed(1) : "0.0",
      avgDuration: metrics.totalRuns > 0 ? Math.round(metrics.totalDuration / metrics.totalRuns) : 0,
