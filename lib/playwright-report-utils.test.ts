...
import { processPlaywrightReportFile, ReportProcessingError } from './playwright-report-utils';
...
describe('playwright-report-utils', () => {
  ...
  describe('processPlaywrightReportFile', () => {
    ...
    it('should extract test status correctly', async () => {
      ...
      const validStatuses = ['passed', 'failed', 'flaky', 'skipped', 'timedOut'];
    ...
    it('should extract error messages for failed tests', async () => {
      ...
      if (failedTests.length > 0) {
        ...
        expect(
          failedTest.error !== undefined || 
          failedTest.errorStack !== undefined
        ).toBe(true);
      }
    });
    ...
    it('should extract screenshot paths', async () => {
      ...
      const testsWithScreenshots = result.tests.filter(t => t.screenshots.length > 0);
    ...
    it('should extract worker index and start time', async () => {
      ...
      const testsWithWorkerInfo = result.tests.filter(t => t.worker_index !== undefined);
