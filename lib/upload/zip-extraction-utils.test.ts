...
describe("zip-extraction-utils", () => {
  ...
  describe("extractTestsFromZip - Integration Tests with Real Data", () => {
    ...
    it("should calculate correct total duration from all retry attempts", async () => {
      ...
      const sumOfRetries = test.attempts.reduce(
        (sum: number, retry: ExtractedTestAttempt) => sum + retry.duration,
        0,
      );
  ...
  describe("Test Steps Extraction", () => {
      ...
      if (firstStep.error) {
        ...
        expect(
          typeof firstStep.error === "string" ||
            (typeof firstStep.error === "object" &&
              "message" in firstStep.error),
        ).toBe(true);
      }
    });
    ...
    it("should handle both string and object error formats in steps", async () => {
      ...
      const stepsWithErrors: NonNullable<
        NonNullable<ExtractedTest["attempts"]>[number]
      >["steps"] = [];
      result.tests.forEach((test) => {
        test.attempts?.forEach((attempt: ExtractedTestAttempt) => {
          if (attempt.steps) {
            const collectStepsWithErrors = (steps: typeof attempt.steps) => {
              steps!.forEach((step) => {
                if (step.error) {
    ...
    it("should include step categories when available", async () => {
      ...
      const allSteps: NonNullable<
        NonNullable<NonNullable<ExtractedTest["attempts"]>[number]>["steps"]
      >[number][] = [];
      result.tests.forEach((test) => {
        test.attempts?.forEach((attempt: ExtractedTestAttempt) => {
          if (attempt.steps) {
            const collectAllSteps = (steps: typeof attempt.steps) => {
              steps!.forEach((step) => {
                allSteps.push(step);
                if (step.steps) {
      ...
      if (allSteps.length > 0) {
        ...
        if (stepsWithCategories.length > 0) {
          const categories = new Set(
            stepsWithCategories.map((s) => s.category),
          );
