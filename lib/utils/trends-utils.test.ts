...
describe("groupRunsByDay - Duration Calculations", () => {
  it("should convert milliseconds to minutes correctly", () => {
    ...
    expect(result).toHaveLength(1);
    expect(result[0].avgDuration).toBe(1); // Should be 1 minute
  });
  ...
  it("should calculate average duration across multiple runs", () => {
    ...
    expect(result).toHaveLength(1);
    // Average: (60000 + 120000 + 90000) / 3 / 60000 = 270000 / 3 / 60000 = 1.5 minutes, rounds to 2
    expect(result[0].avgDuration).toBe(2);
  });
  ...
  it("should round duration to nearest second", () => {
    ...
    expect(result).toHaveLength(1);
    // Average: (65499 + 65500) / 2 / 60000 = 130999 / 2 / 60000 = 1.09, rounds to 1
    expect(result[0].avgDuration).toBe(1);
  });
  ...
  it("should handle realistic test suite durations", () => {
    ...
    expect(result).toHaveLength(1);
    // 4186000 / 60000 = 69.77 minutes, rounds to 70
    expect(result[0].avgDuration).toBe(70);
  });
  ...
  it("should calculate separate averages for different days", () => {
    ...
    expect(result).toHaveLength(2);
    expect(result[0].avgDuration).toBe(1); // 60000 / 60000 = 1 minute
    expect(result[1].avgDuration).toBe(2); // 120000 / 60000 = 2 minutes
  });
  ...
  it("should handle mixed runs with and without duration", () => {
    ...
    expect(result).toHaveLength(1);
    // Average: (60000 + 0 + 90000) / 3 / 60000 = 0.83 minutes, rounds to 1
    expect(result[0].avgDuration).toBe(1);
  });
});
