...
async function migrateBase64ToStorage() {
  ...
  for (let i = 0; i < base64Tests.length; i += BATCH_SIZE) {
    ...
    console.log(`📦 Processing batch ${Math.floor(i / BATCH_SIZE) + 1}/${Math.ceil(base64Tests.length / BATCH_SIZE)} (${batch.length} tests)...`);
    ...
    for (const test of batch) {
      ...
      for (let idx = 0; idx < test.screenshots.length; idx++) {
        ...
        try {
          ...
          if (!matches) {
            console.warn(`   ⚠️  Invalid base64 format for test ${test.id}, screenshot ${idx}`);
            errors++;
          ...
          if (DRY_RUN) {
            console.log(`   [DRY RUN] Would upload screenshot ${idx + 1}/${test.screenshots.length} for test ${test.id}`);
            migratedScreenshots++;
            continue;
          }
          ...
          if (uploadError) {
            console.error(`   ❌ Failed to upload screenshot for test ${test.id}:`, uploadError.message);
            errors++;
          ...
          if (signedUrlError) {
            console.error(`   ❌ Failed to generate signed URL for test ${test.id}:`, signedUrlError.message);
            errors++;
            // Try to delete the uploaded file
            await supabase.storage.from('test-screenshots').remove([storagePath]);
            newScreenshots.push(screenshot); // Keep original on error
            continue;
          }
          ...
          console.error(`   ❌ Error processing screenshot for test ${test.id}:`, error);
          errors++;
          newScreenshots.push(screenshot); // Keep original on error
        }
      }
      ...
      if (!DRY_RUN && newScreenshots.length > 0) {
        ...
        if (updateError) {
          console.error(`   ❌ Failed to update test ${test.id}:`, updateError.message);
          errors++;
        }
      }
    }
  ...
  if (!testResultsError && testResults) {
    ...
    console.log(`   Found ${base64TestResults.length} test_results with base64 screenshots`);
    console.log('');
